...
import com.stryse.ImageProcessingStation;
import com.stryse.Tools.*;
...
/* *** Story time ***
  ...
  takes the following 3 steps:
    1. John preprocesses the image (contrast enhancement and other fancy
    ...
    3. Finally, once motion is detected John can send the processed images
       ...
       communication, so the speed of this operation is independent of the
...
public class Pipe {
  ...
  */
  ...
  static void safeObjectDetectWrapper(IMG img){
    try{
      img.objectDetect();
      ++counter;
    } catch (Exception e){
  ...
  public static Tools.IMG[] mark5(Tools.IMG[] imgs) {
    ...
    final BlockingQueue<IMG> toBePreprocessed = new LinkedBlockingQueue<>(Arrays.asList(imgs));
    final BlockingQueue<IMG> toBeMotionDetected = new LinkedBlockingQueue<>();
    final BlockingQueue<IMG> toBeObjectDetected = new LinkedBlockingQueue<>();
    ...
    //Preprocess
    for(int i = 0; i < ppThreads; ++i)
      exec.submit(new ImageProcessingStation(toBePreprocessed,toBeMotionDetected,IMG::preProcess,()->toBePreprocessed.isEmpty()));

    //Motion Detect
    for(int i = 0; i < mdThreads; ++i)
      exec.submit(new ImageProcessingStation(toBeMotionDetected,toBeObjectDetected,IMG::motionDetect,()->toBePreprocessed.isEmpty() && toBeMotionDetected.isEmpty()));

    //Object Detect
    ImageProcessingStation odStation = new ImageProcessingStation(toBeObjectDetected,null,Pipe::safeObjectDetectWrapper,()->counter == imgs.length);
    odStation.run();
